...
product docs to a content hub built with the `astro-content-hub` template.
...
dimension is injected at sync time from the `PRODUCT` env var (see "Syncing"), so the
...
<external-project>/
  posts/
    en/
      ...
      my-product/
        2025-07-20-why-my-product.md   <- nested dirs become path segments
    zh-Hans/
...
author: "Your Name"                     # optional, string
...
| `posts/en/my-product/2025-07-20-why-my-product.md` | `my-product/2025-07-20-why-my-product` |
...
- A `zh-Hans` page that is missing still has a URL (`/zh-Hans/.../`) on the hub;
  that URL renders the `en` body inside the Chinese shell, with a visible notice
...
- In an `en` post/doc, link to other pages with their default paths:
  `/posts/foo/`, `/<product>/docs/bar/`.
- In a `zh-Hans` post/doc, use the `/zh-Hans/` prefix so readers stay in the
  Chinese shell: `/zh-Hans/posts/foo/`, `/zh-Hans/<product>/docs/bar/`.
...
```bash
node skills/site-content-sync/scripts/validate.mjs
```
...
write access to the hub repo.

### 1. Create the PAT (one-time, on the hub side)

Create a fine-grained PAT (or a GitHub App token) on `<HUB_REPO>` with
**Contents: write** (to push the sync branch) **and Pull requests: write**
(to open the PR). Add it as a repository secret named `<HUB_TOKEN_SECRET>`
in the **external** project (Settings → Secrets and variables → Actions →
New repository secret).

### 2. Add the workflow(s)

Copy from `skills/site-content-sync/templates/`:
...
1. Open a PR (or issue) against `<HUB_REPO>` adding the product entry to
   ...
   `skills/site-content/SKILL.md` documents this for whoever owns the hub
   repo.
