...
```ts
const products = ['vite', 'astro', 'json-server', 'mytool'] as const;
```
...
1. Open a PR (or issue) against `<HUB_REPO>` adding the product name to
   `products` and creating the route files (en + zh, index + catch-all) under
   `src/pages/<product>/docs/` and `src/pages/zh/<product>/docs/`. The hub's
   own `skills/site-content/SKILL.md` documents this for whoever owns the hub
   repo.
